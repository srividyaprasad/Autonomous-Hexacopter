...
bool px4_msgs__msg__airspeed_wind__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // tas_scale
    PyObject * field = PyObject_GetAttrString(_pymsg, "tas_scale");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->tas_scale = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // beta_innov
...
PyObject * px4_msgs__msg__airspeed_wind__convert_to_py(void * raw_ros_message)
{
  ...
  {  // tas_scale
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->tas_scale);
    {
      int rc = PyObject_SetAttrString(_pymessage, "tas_scale", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
