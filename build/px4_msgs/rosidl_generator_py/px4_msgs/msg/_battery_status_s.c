...
bool px4_msgs__msg__battery_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // average_current_a
    PyObject * field = PyObject_GetAttrString(_pymsg, "average_current_a");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->average_current_a = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // discharged_mah
...
PyObject * px4_msgs__msg__battery_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // average_current_a
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->average_current_a);
    {
      int rc = PyObject_SetAttrString(_pymessage, "average_current_a", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
