...
class GpsInjectData(metaclass=Metaclass_GpsInjectData):
    ...
    SLOT_TYPES = (
        ...
        rosidl_parser.definition.BasicType('uint8'),  # noqa: E501
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('uint8'), 182),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        if 'data' not in kwargs:
            self.data = numpy.zeros(182, dtype=numpy.uint8)
        else:
            self.data = numpy.array(kwargs.get('data'), dtype=numpy.uint8)
            assert self.data.shape == (182, )
    ...
    def data(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 182, \
                "The 'data' numpy.ndarray() must have a size of 182"
            self._data = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'data' field must be a set or sequence with length 182 and each value of type 'int' and each unsigned integer in [0, 255]"
