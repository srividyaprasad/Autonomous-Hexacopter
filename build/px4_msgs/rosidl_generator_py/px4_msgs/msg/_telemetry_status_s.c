...
bool px4_msgs__msg__telemetry_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // rx_message_count_supported
    PyObject * field = PyObject_GetAttrString(_pymsg, "rx_message_count_supported");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->rx_message_count_supported = PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
PyObject * px4_msgs__msg__telemetry_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // rx_message_count_supported
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->rx_message_count_supported);
    {
      int rc = PyObject_SetAttrString(_pymessage, "rx_message_count_supported", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
