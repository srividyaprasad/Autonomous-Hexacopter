...
bool px4_msgs__msg__vehicle_constraints__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // speed_xy
    PyObject * field = PyObject_GetAttrString(_pymsg, "speed_xy");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->speed_xy = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // speed_up
...
PyObject * px4_msgs__msg__vehicle_constraints__convert_to_py(void * raw_ros_message)
{
  ...
  {  // speed_xy
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->speed_xy);
    {
      int rc = PyObject_SetAttrString(_pymessage, "speed_xy", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
