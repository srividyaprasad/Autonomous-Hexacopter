...
bool px4_msgs__msg__vehicle_imu_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // gyro_coning_vibration
    PyObject * field = PyObject_GetAttrString(_pymsg, "gyro_coning_vibration");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->gyro_coning_vibration = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // mean_accel
...
PyObject * px4_msgs__msg__vehicle_imu_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // gyro_coning_vibration
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->gyro_coning_vibration);
    {
      int rc = PyObject_SetAttrString(_pymessage, "gyro_coning_vibration", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
