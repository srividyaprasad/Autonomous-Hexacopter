...
{
  const auto & member =
    *reinterpret_cast<const std::array<uint8_t, 182> *>(untyped_member);
  return &member[index];
}
...
{
  auto & member =
    *reinterpret_cast<std::array<uint8_t, 182> *>(untyped_member);
  return &member[index];
}

static const ::rosidl_typesupport_introspection_cpp::MessageMember GpsInjectData_message_member_array[4] = {
  {
    "timestamp",  // name
